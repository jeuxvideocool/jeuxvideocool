import{a as Xe,b as Ne,d as Re,e as Ge,f as Je,o as Fe,g as fe,w as ye,u as le,c as Oe,h as De,i as qe,r as He,j as _e,k as ze,A as Ve}from"./index-CsvP8RUZ.js";import{subscribe as We,getAuthState as Ke,connectCloud as _,uploadAvatarImage as Ye,removeAvatarImage as Qe,requestCloudResetSync as Ze,saveCloud as be,syncCloudToLocal as es,getAvatarPublicUrl as de}from"./cloud-CQZoWt0Z.js";const re=document.getElementById("app"),w=Xe(),D=Ne(),$e=Re(),pe=Ge();let C="hub",m,ie=1,ue=!1,o=Ke(),Q="",G="all",O=!1,U=null,z=null,B=!1;function Pe(){var a;const e=o.user;if(!e)return"connecté";const s=(a=e.user_metadata)==null?void 0:a.identifier,t=e.email;return s||(t!=null&&t.endsWith("@user.local")?t.replace("@user.local",""):t||"connecté")}function xe(e){return e?"Image stockée sur Supabase.":o.ready?o.user?"Choisis une image, elle sera envoyée sur Supabase.":"Connecte-toi au cloud pour utiliser une image.":"Supabase non configuré (.env)."}Je();Ee(Se(w.hubTheme));We(e=>{o=e,M()});Fe("ACHIEVEMENT_UNLOCKED",e=>{var a;const s=(a=e.payload)==null?void 0:a.achievementId,t=D.achievements.find(i=>i.id===s);t&&y(`Succès débloqué : ${t.title}`,"success"),s===Ve.achievementId&&M()});function Se(e){return e?pe.find(s=>s.id===e):pe[0]}function Ee(e){if(!e)return;const s=document.documentElement.style;s.setProperty("--color-primary",e.colors.primary),s.setProperty("--color-secondary",e.colors.secondary),s.setProperty("--color-accent",e.colors.accent),s.setProperty("--color-bg",e.colors.background),s.setProperty("--color-surface",e.colors.surface),s.setProperty("--color-text",e.colors.text),s.setProperty("--color-muted",e.colors.muted),e.gradient&&s.setProperty("--hero-gradient",e.gradient)}function y(e,s="info"){const t=document.createElement("div");t.className=`toast ${s}`,t.textContent=e,document.body.appendChild(t),requestAnimationFrame(()=>t.classList.add("visible")),setTimeout(()=>{t.classList.remove("visible"),setTimeout(()=>t.remove(),300)},2600)}function Z(e){return e?new Date(e).toLocaleString("fr-FR",{dateStyle:"medium",timeStyle:"short"}):"Jamais"}function A(e){if(!e)return"0m";const s=Math.floor(e/1e3),t=Math.floor(s/3600),a=Math.floor(s%3600/60),i=s%60;return t?`${t}h ${a}m`:a?`${a}m ${i}s`:`${i}s`}function R(e){return new Intl.NumberFormat("fr-FR").format(e)}function ss(e){return e.trim().slice(0,3).toUpperCase()}function ve(e,s){if(e.length<2)return null;const t=e.map(c=>Number.isFinite(c)?c:0),a=100,i=42,r=4,x=Math.max(...t,1),h=Math.min(...t,0),b=x-h||1,P=(a-r*2)/Math.max(1,t.length-1),n=t.map((c,p)=>{const $=r+P*p,N=(c-h)/b,S=r+(i-r*2)*(1-N);return{x:$,y:S}}),f=n.map(c=>`${c.x.toFixed(2)},${c.y.toFixed(2)}`).join(" "),v=`${r},${i-r} ${f} ${a-r},${i-r}`,T=s===void 0?n.length-1:s,L=Math.max(0,Math.min(T,n.length-1)),g=Math.floor(L),j=L-g,E=n.slice(0,g+1);if(j>0&&g<n.length-1){const c=n[g],p=n[g+1];E.push({x:c.x+(p.x-c.x)*j,y:c.y+(p.y-c.y)*j})}const k=E.map(c=>`${c.x.toFixed(2)},${c.y.toFixed(2)}`).join(" "),d=E[E.length-1];return{points:f,area:v,progress:k,marker:d}}function me(e,s){return e?`
  ...
  `}function ms(){const e=w.games.length?[]:["games.registry.json vide ou invalide"],s=new Set(m.save.favorites||[]),t=Array.from(new Set(w.games.flatMap(n=>n.tags||[]).filter(Boolean))).sort((n,f)=>n.localeCompare(f,"fr")),a=Q.trim().toLowerCase(),i=Q.replace(/"/g,"&quot;"),r=w.games.filter(n=>O?s.has(n.id):!0).filter(n=>G==="all"?!0:(n.tags||[]).includes(G)).filter(n=>a?n.title.toLowerCase().includes(a)||n.description.toLowerCase().includes(a)||n.id.toLowerCase().includes(a):!0).sort((n,f)=>{const v=Number(s.has(f.id))-Number(s.has(n.id));return v!==0?v:(n.order??0)-(f.order??0)}),x=r.map(n=>{$e.find(k=>k.id===n.id)||e.push(`Config manquante pour ${n.id}`);const v=m.save.games[n.id],T=v!=null&&v.lastPlayedAt?Z(v.lastPlayedAt):"Jamais",L=(v==null?void 0:v.bestScore)??null,g=A(v==null?void 0:v.timePlayedMs),j=ye(`/apps/games/${n.id}/`),E=s.has(n.id);return`
