import{w as v,A as m,g as S,c as R}from"./index-CsvP8RUZ.js";import{getAuthState as k,subscribe as C}from"./cloud-CQZoWt0Z.js";const y=document.getElementById("app");let c=k();const b=e=>e[Math.floor(Math.random()*e.length)];function T(){y.innerHTML=`
    <div class="page">
      <main class="shell">
        <header class="hero">
  ...
  `}function I(e){var r;if(!e)return null;const i=(r=e.user_metadata)==null?void 0:r.identifier;if(i)return i;const a=e.email||"";if(!a)return null;if(a.endsWith("@user.local"))return a.replace("@user.local","");const n=a.indexOf("@");return n>0?a.slice(0,n):a}function X(e){const i=I(e);return(i==null?void 0:i.trim().toLowerCase())===m.requiredName.trim().toLowerCase()}function E(){const i=S().save;if(!R(i)){window.location.replace(v("/"));return}const a="Alexiane",n=i.playerProfile.avatar||"💫",r=i.globalXP.toLocaleString("fr-FR"),o=m.minXP.toLocaleString("fr-FR"),d=Math.min(100,Math.max(12,Math.round(i.globalXP/(m.minXP*1.25)*100))),l=new Date().toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"}),u=["Tu viens de passer en mode prestige : plus de style, zéro filtre.","On a tenté de faire sobre. Le code a répondu : « Alexiane, impossible ».","C'est la page secrète qui dit haut et fort que tu as le swag intégré."],f=["Vote unanime : garder Alexiane au sommet, ajouter une dose de fun et signer tout de suite.","Après examen, on confirme : niveau charme 9000, option premium activée.","Résultat du scan : rareté maximale, humour calibré, classe automatique."],p=["Si quelqu'un demande comment tu as débloqué ça, réponds « secret de fabrication ».","Tu peux revenir ici quand tu veux, c'est ton lounge privé.","Attention : cette page peut provoquer des sourires incontrôlables."],h=["Sourire naturel : activé","Style : premium","Vibes : stables","Punchlines : prêtes"],g=["Édition 1/1","Validé par le comité","Premium certifié"],x=v("/");y.innerHTML=`
    <div class="page">
  ...
  `,D()}function D(){const e=document.getElementById("fireworks");if(!e||window.matchMedia("(prefers-reduced-motion: reduce)").matches)return;const a=e.getContext("2d");if(!a)return;let n=0,r=0,o=1;const d=["#ffd27a","#6de3ff","#ff89c4","#ffe96b","#8dffb0"],l=[],u=()=>{o=Math.min(2,window.devicePixelRatio||1),n=window.innerWidth,r=window.innerHeight,e.width=Math.round(n*o),e.height=Math.round(r*o),e.style.width=`${n}px`,e.style.height=`${r}px`,a.setTransform(o,0,0,o,0,0)},f=(t,s,M=1)=>{const L=Math.round(60*M);for(let P=0;P<L;P+=1){const A=Math.random()*Math.PI*2,$=(Math.random()*3+2.2)*M,q=Math.random()*2+1.4;l.push({x:t,y:s,vx:Math.cos(A)*$,vy:Math.sin(A)*$,alpha:1,decay:.012+Math.random()*.016,size:q,color:d[Math.floor(Math.random()*d.length)]})}};let p=!1;const h=()=>{a.clearRect(0,0,n,r),a.globalCompositeOperation="lighter";for(let t=l.length-1;t>=0;t-=1){const s=l[t];if(s.vy+=.04,s.vx*=.98,s.vy*=.98,s.x+=s.vx,s.y+=s.vy,s.alpha-=s.decay,s.alpha<=0){l.splice(t,1);continue}a.globalAlpha=s.alpha,a.fillStyle=s.color,a.beginPath(),a.arc(s.x,s.y,s.size,0,Math.PI*2),a.fill()}l.length>0?requestAnimationFrame(h):p=!1},g=()=>{p||(p=!0,requestAnimationFrame(h))};u(),window.addEventListener("resize",u),[{delay:0,x:.2,y:.32,power:1.2},{delay:180,x:.5,y:.22,power:1.4},{delay:360,x:.8,y:.3,power:1.2},{delay:700,x:.35,y:.45,power:1.05},{delay:920,x:.68,y:.4,power:1.15}].forEach(t=>{window.setTimeout(()=>{f(n*t.x,r*t.y,t.power),g()},t.delay)})}c.ready?c.user?X(c.user)?c.hydrated?E():(T(),C(e=>{c=e,c.user&&c.hydrated&&window.location.reload()})):w("Tu n'es pas connecté avec le compte Alex."):w("Connecte-toi au cloud depuis le hub pour déverrouiller l'accès."):w("Supabase n'est pas configuré pour vérifier l'identité.");
